module.exports = async (req, res) => {
  ...
  try {
    ...
    // 1. Busca os produtos legítimos no banco de dados Supabase para validar preço real
    let dbProducts = [];
    try {
    ...
    // 2. Calcula o subtotal legítimo baseado no banco ou fallback seguro
    let subtotal = 0;
    const verifiedItems = [];
    ...
    // 3. Validação segura de cupons no backend
    const validCoupons = {
      "ZIVA10": 10,
      "PRIMEIRACOMPRA": 15,
      "ZIVA20": 20,
    ...
    // 4. Monta payload de pagamento PIX para o Mercado Pago
    const mpPayload = {
      ...
      payer: {
        ...
        last_name: payer.last_name || "Ziva",
    ...
    // 5. Envia requisição oficial ao Mercado Pago
